/**
 * @file apps/line-client/src/app/api/rankings/weekly/route.ts
 * @description 週間ランキングを取得するAPIエンドポイント
 */
...
// ランキングクエリの型定義（簡略化のため）
type RankingQuery = {
  from: (table: string) => RankingQuery;
  select: (query: string, options?: { count: string }) => RankingQuery;
  eq: (column: string, value: any) => RankingQuery;
  gte: (column: string, value: any) => RankingQuery;
  lte: (column: string, value: any) => RankingQuery;
  order: (column: string, options: { ascending: boolean }) => RankingQuery;
  range: (from: number, to: number) => RankingQuery;
};
