/**
 * @file packages/api/src/handlers/rankings/all-time.ts
 * @description 総合ランキングを取得するハンドラー
 */
...
export async function getAllTimeRankings(request: NextRequest) {
  try {
    ...
    const { error: apiError, statusCode } = handleError(error, 'GET /api/rankings/all-time');
