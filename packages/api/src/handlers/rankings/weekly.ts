/**
 * @file packages/api/src/handlers/rankings/weekly.ts
 * @description 週間ランキングを取得するハンドラー
 */
...
export async function getWeeklyRankings(request: NextRequest) {
  try {
    ...
    const { error: apiError, statusCode } = handleError(error, 'GET /api/rankings/weekly');
